...
/*   Updated: 2022/02/12 14:18:56 by jaka          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	is_dead(t_philo *philo, unsigned long long curr_time)
{
	...
	{
		pthread_mutex_lock(&philo->main->mutex_write);
		printf("%lld %d %s", timestamp(philo), philo->pos, "died");
	...
	if (philo->main->meals == philo->main->nb_philo)
	{
		...
		pthread_mutex_destroy(&philo->main->mutex_write); // added jaka
